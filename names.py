"""Name things with words from Oren Tirosh's mnemonic word list.

How many things do you need to name?

If it's less than `len(names)` (1633), then you're good.
"""
...
def sample_names(
    ...
    """
    random.seed(seed)
    return random.sample(names, len(names))[start:stop]


# Names come from Oren Tirosh's mnemonic word list:
# https://web.archive.org/web/20091003023412/http://tothink.com/mnemonic/wordlist.txt
names = """   Wordlist ver 0.732 - EXPECT INCOMPATIBLE CHANGES;
...
if __name__ == "__main__":
    ...
    print("\n".join(sample_names(args.seed, args.start, args.end)))
